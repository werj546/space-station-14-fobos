...
public sealed partial class AtmosphereSystem
{
        ...
        TileAtmosphere tile)
    {
        ...
        if (tile.Hotspot.Temperature < Atmospherics.FireMinimumTemperatureToExist ||
            ...
            tile.Air.GetMoles(Gas.Plasma) < 0.5f && tile.Air.GetMoles(Gas.Tritium) < 0.5f && tile.Air.GetMoles(Gas.Hydrogen) < 0.5f) // DS14
        {
        ...
        EntityUid? sparkSourceUid = null)
    {
        ...
        var hydrogen = tile.Air.GetMoles(Gas.Hydrogen); // DS14

        if (tile.Hotspot.Valid)
        {
            if (soh)
            {
                if (plasma > 0.5f || tritium > 0.5f || hydrogen > 0.5f) // DS14
                {
        ...
        if (exposedTemperature > Atmospherics.PlasmaMinimumBurnTemperature && (plasma > 0.5f || tritium > 0.5f || hydrogen > 0.5f)) // DS14
        {
            if (sparkSourceUid.HasValue)
            {
                _adminLog.Add(LogType.Flammable,
                    LogImpact.High,
                    $"Heat/spark of {ToPrettyString(sparkSourceUid.Value)} caused atmos ignition of gas: {tile.Air.Temperature.ToString():temperature}K - {oxygen}mol Oxygen, {plasma}mol Plasma, {tritium}mol Tritium, {hydrogen}mol Hydrogen"); // DS14
            }
