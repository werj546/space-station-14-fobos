...
using Content.Shared.DeadSpace.Ports.TapeRecorder;
using Content.Shared.DeadSpace.Ports.TapeRecorder.Components;
using Content.Shared.DeadSpace.Ports.TapeRecorder.Events;
...
public sealed partial class TapeRecorderSystem : SharedTapeRecorderSystem
{
    ...
    private void OnListen(Entity<TapeRecorderComponent> ent, ref ListenEvent args)
    {
        ...
        string voiceId = SharedHumanoidAppearanceSystem.DefaultTapeRecorderVoice;

        if (TryComp<TTSComponent>(args.Source, out var ttsComp) && ttsComp.VoicePrototypeId != null)
        {
            var voiceEv = new TransformSpeakerVoiceEvent(args.Source, ttsComp.VoicePrototypeId);
