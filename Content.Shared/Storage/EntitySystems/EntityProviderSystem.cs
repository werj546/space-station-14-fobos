...
public sealed partial class EntityProviderSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        _providerQuery = GetEntityQuery<EntityProviderComponent>();
        _storageQuery = GetEntityQuery<StorageComponent>();

        SubscribeLocalEvent<EntityProviderComponent, ComponentInit>(OnInit);
        SubscribeLocalEvent<EntityProviderComponent, AfterInteractEvent>(OnAfterInteract);
        SubscribeLocalEvent<EntityProviderComponent, InteractUsingEvent>(OnInteractUsing);
    }
    // DS14-end
    ...
    private bool TryFillOtherProvider(Entity<EntityProviderComponent> provider, Entity<EntityProviderComponent> refillTarget, EntityUid? user = null)
    {
        ...
        {
            if (IsPrototypeWhitelistFail(refillTarget.Comp.Whitelist, entProtoId))
                continue;
    ...
    private bool IsPrototypeWhitelistFail(EntityWhitelist whitelist, EntProtoId protoId)
    {
        ...
        {
            foreach (var componentName in whitelist.Components)
            {
                var hasComponent = Factory.TryGetRegistration(componentName, out var registration)
                    && prototype.Components.ContainsKey(registration.Name);

                if (hasComponent && !whitelist.RequireAll)
                    return false;
