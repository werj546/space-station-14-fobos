...
public sealed partial class XATInteractAttackSystem : BaseXATSystem<XATInteractAttackComponent>
{
    ...
    public override void Initialize()
    {
        ...
        // DS14-start: attribute subscriptions are unavailable on the pinned API.
    ...
    private bool DoTriggerCountdown(Entity<XATInteractAttackComponent> ent, EntityUid artifact, EntityUid? user)
    {
        ...
        if (ent.Comp.Count > 0)
        {
            // DS14-start: the current targeted popup overload requires a non-null recipient.
            if (ent.Comp.InsufficientInteractionPopup != null && user is { } recipient)
                _popup.PopupEntity(Loc.GetString(ent.Comp.InsufficientInteractionPopup), artifact, recipient); //tell user they need to interact in the same way more times
            // DS14-end
