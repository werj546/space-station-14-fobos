...
public sealed partial class PanicBunkerTab : Control
{
    ...
    public void UpdateStatus(PanicBunkerStatus status)
    {
        ...
        SetNegativeStyle(EnabledButton, status.Enabled); // DS14
    ...
    // DS14-start
    private static void SetNegativeStyle(Control control, bool enabled)
    {
        if (enabled)
        {
            if (!control.StyleClasses.Contains(StyleClass.Negative))
                control.AddStyleClass(StyleClass.Negative);

            return;
        }

        control.RemoveStyleClass(StyleClass.Negative);
    }
    // DS14-end
}
