...
public sealed partial class PhotocopierWindow : FancyWindow
{
    ...
    private ProtoId<PaperworkFormPrototype>? _chosenForm;
    private Button? _selectedFormButton;
        ...
        foreach (var prototype in forms)
        {
                ...
                search.Length == 0 && _category != null && prototype.Category.Equals(_category))
            {
                if (_categories.Contains(prototype.Category))
                {
                    ...
                    button.AddStyleClass(DeadSpaceStyleClass.ListItem);
                    button.Text = Loc.GetString(prototype.Name);
                    button.ToggleMode = true;
                    button.Pressed = prototype.ID == _chosenForm;
                    if (button.Pressed)
                        _selectedFormButton = button;
                    button.OnPressed += _ => SelectForm(button, prototype.ID);
    ...
    private void SelectForm(Button button, ProtoId<PaperworkFormPrototype> form)
    {
        if (_selectedFormButton != null && _selectedFormButton != button)
            _selectedFormButton.Pressed = false;
